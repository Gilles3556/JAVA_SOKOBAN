    ...
    public  void initWorld() {
        ...
        for (int i = 0; i < level.length(); i++) {
            ...
            switch (item) {
                ...
                case '\\':
                    // avant le  test de faisait sur \n
                    // charAt(i) donc n'interczepte que '\' sur '\n'
                    y += C.SPACE;
                    if (w < x) {
                        w = x;
                    }
                ...
                case '#':
                    wall = new Wall(x, y);
                    System.out.println("MUR en y:"+y+"; x="+x);
                    walls.add(wall);
                    x += C.SPACE;
                    break;

                case '$':
                    System.out.println("BAGAGE en y:"+y+"; x="+x);
                ...
                case '@':
                    System.out.println("SOKO en y:"+y+"; x="+x);
                    soko = new Player(x, y);
                    x += C.SPACE;
                    break;
