...
import pndg.v3.model.TypeActeur;
...
public class FabriqueIhm {
    ...
    public static Image getImageForActeur( TypeActeur leType ){
        ...
        if (leType==TypeActeur.AREA){
            iicon = new ImageIcon("src/resources/sokoban_ress/area.gif");
            uneImage = iicon.getImage();
        }
        if(leType==TypeActeur.BAGGAGE){
            iicon = new ImageIcon("src/resources/sokoban_ress/baggage.gif");
            uneImage = iicon.getImage();
        }
        if(leType==TypeActeur.PLAYER){
            iicon = new ImageIcon("src/resources/sokoban_ress/9.gif");
            uneImage = iicon.getImage();
        }
       if(leType==TypeActeur.WALL){
           iicon = new ImageIcon("src/resources/sokoban_ress/wall.gif");
           uneImage = iicon.getImage();
       }
        return uneImage;
    }
